    ...
    protected function multiConnect($master=false) {
        ...
        if(DB_RW_SEPARATE){
            ...
            if($master)
                ...
                if(is_numeric(DB_SLAVE_NO)) {// 指定服务器读
                    $r = DB_SLAVE_NO;
                }else{
                    // 读操作连接从服务器
                    $r = floor(mt_rand(DB_MASTER_NUM,count($_config['hostname'])-1));   // 每次随机连接的数据库
                }
            }
        }else{
            // 读写操作不区分服务器
...
//        }
    ...
    protected function parseTable($tables) {
        ...
        $tables = preg_replace_callback("/__([A-Z_-]+)__/sU", function($match){ return DB_PREFIX.strtolower($match[1]);}, implode(',',$tables));
        return $tables;
    }
...
//                    E(L('_EXPRESS_ERROR_').':'.$val[0]);
    ...
    protected function parseJoin($join) {
        ...
        if(!empty($join)) {
            ...
            $joinStr  = preg_replace_callback("/__([A-Z_-]+)__/sU", function($match){ return DB_PREFIX.strtolower($match[1]);}, $joinStr);
        }
        return $joinStr;
    }
...
//            $value  =  S($key);
        ...
        if(isset($key)) { // 写入SQL创建缓存
            $cach = Yii::$app->cache; 
           $cach->set($key,$sql,array('expire'=>0,'length'=>DB_SQL_BUILD_LENGTH,'queue'=>DB_SQL_BUILD_QUEUE));
