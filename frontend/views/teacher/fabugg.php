    ...
    <div style="background-color: white;padding: 10px 20px;box-shadow: 0 0 2px #ccc;">
        ...
        <hr/>
            <p class="text-primary row">
                <span class="badge col-lg-1" style="background: red;padding: 3px 10px">公告</span>
                <span class="col-lg-1"></span>
                                <span class="col-lg-10">
                                    <input name="title" style="margin-top: -10px;margin-bottom: 10px;" class="form-control" placeholder="请输入主题信息" value="<?php echo !empty($info[0]['title'])?$info[0]['title']:"" ?>" />
                                </span>
            </p>
            <div>
    ...
    function tijiao(t)
    {
        if (checkinfo())
        {
            ...
            $.post(url, formdata).done(function (data) {
                if (data == 0){
                    alert("发布成功");
                    window.location = "/teacher/schoolnotice";
                }else {
                    alert("发布失败");
                }
            });
...
//        var arr = [];
//        UE.getEditor('editor').setContent("454574", isAppendTo);
//    }
