...
<nav id="cd-lateral-nav"> <!-- 通讯录 -->
  <div class="container-fluid margin-footer">
    <div id="display-list" style="margin-top:50px;"> <!-- display-list 设置页面的显示 和 隐藏 -->
      ...
      <div class="row user-search" style="margin-left:1%;">
...
<!--        <foreach name="list_lxr" item="vo" key="key">-->
    <?php foreach( $list_lxr as $k=>$v){?>
          ...
          <div class="user-list-row"  onclick="user_checkbox(this,event,<?= $v['bending']?>,<?= $v['endjx']?>)">
            <div class="col-xs-9"><?php echo $v['name'] ?>家长
              <?php if($v['bending']==0) {?>
               (未绑定)
              <?php }?>
              <?php if($v['endjx']==0) {?>
            ...
            <div class="col-xs-3">
               <?php if($v['bending']==0 || $v['endjx']==0) {?>
              <div class="checkbox" style="background-color:#ccc;">
                <input type="checkbox" value="<?php echo $v['id'] ?>" disabled='disabled' name="<?php echo $v['name'] ?>" id="u<?php echo $v['name'] ?>" class="lxr" />
...
function user_checkbox(ths,event,bending,endjx){
