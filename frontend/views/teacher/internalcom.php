                                ...
                                <div id="collapse<?= $key; ?>" class="panel-collapse collapse <?php if ($key == 0) {
                                    ...
                                    <div class="panel-body" id="tupian">
                                        ...
                                        <div style="margin: 20px;">
                                            <button class="btn zd_btn2" id="<?= $value['id']; ?>"><a
                                                    href="/teacher/delinbox?id=<?= $value['id']; ?>&url=internalcom">删除</a>
                                            </button>
                                            <button class="btn zd_btn2" value=""><a href="###"
                                                                                    name="<?= $value['outopenid']; ?>"
                                                                                    id="<?= $value["title"] ?>"
                            ...
                            <div id="Yf<?=$key?>" class="panel-collapse collapse <?php if($key==0){echo ' in';}; ?>">
                                <div class="panel-body" id="tupian">
                                   ...
                                   <?php $data= substr($value['fujian'],0,4); $data2 =substr($value["fujian"], 4);$fujian = explode('#',$data2); if (!empty($data) && $data == "one#") {foreach ($fujian as $key => $val) {
                                        ...
                                        <?php }} ?>
                                    <div style="margin: 20px;">
                                        <button class="btn zd_btn2"><a href="/teacher/deloutbox?id=<?=$value['id'];?>&url=internalcom">删除</a></button>
                                        <div style="display: none" id="zhuanfat"><?= $value["content"]; ?></div>
                                        <button class="btn zd_btn2"  onclick="zhuanfa(this)">转发</button>
                                    </div>
                                </div>
...
function quanxua(){
    ...
    function shouxinpa(){
        ...
        window.location.href = "/teacher/internalcom?type=shouxin";
    }

    function yifapa(){
        var url = window.location.href;
        window.location.href = "/teacher/internalcom?type=yifa";
    }
    function xiexinpa(){
        var url = window.location.href;
        window.location.href = "/teacher/internalcom?type=xiexin";
    }
