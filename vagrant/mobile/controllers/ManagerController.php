...
class ManagerController extends \mobile\controllers\BaseController
{
	...
	private function downKaoqinExcel($sid, $from_time, $to_time) {
		...
		$sql = "select b.class,b.name,from_unixtime(a.ctime,'%Y-%m-%d %H:%i:%s') ts,a.info from wp_ischool_safecard a " . " left join wp_ischool_student b on b.id=a.stuid where b.sid=" . $sid . " and " . " a.ctime>" . $from_time . " and  a.ctime<" . $to_time . " order by b.class,b.name,a.ctime asc ";
		//$data = M ()->query ( $sql );
	...
	private function downKaoqinhzExcel($sid,$from_time,$to_time){
		...
		COUNT(CASE WHEN info = '进校' THEN 1
		ELSE NULL
		END) 进校总数,
		COUNT(CASE WHEN info = '出校' THEN 2
		ELSE NULL
		END) 出校总数,
		COUNT(CASE WHEN info = '进宿舍' THEN 3
		ELSE NULL
		END) 进宿舍总数,
		COUNT(CASE WHEN info = '出宿舍' THEN 4
		ELSE NULL
		END) 出宿舍总数
	...
	public function actionAllclass()
	{
		...
		$res = WpIschoolClass::find()->where($where)->limit($num)->offset($star)->orderBy('level,class')->asArray()->all();
	...
	public function actionAlluser()
	{
		...
		$nm=\mobile\models\WpIschoolTeacher::find()->where($con)->count();
		...
		$res = \mobile\models\WpIschoolTeacher::find()
		->where($con)
	...
	public function actionCheckuser(){
		...
		$res = $d->save(false);
                ...
                $p= WpIschoolTeaclass::find()->select('openid,sid')->where($con)->asArray()->one();
		...
		SendMsg::sendSHMsgToPa($openid,$title,$des);
		if($res){
	...
	public function GiveEpcrole($pl,$user,$sid){
		...
		if(empty($arr)){
			...
			if($pin){
				$openid=$user;
				$msg="尊敬的".$ar[0]['tel']."用户,您已成为".$school."的5、平安通知管理员，您登录平安卡系统的用户名为：".$ar[0]['tel']."密码为：".$pwd;
				$data = '{
                        "touser":"'.$openid.'",
                        "msgtype":"text",
                         "text":
	...
	public function CancelEpc($pid,$user,$sid){
		...
		if($arr){
			...
			if($da){
				$open=$user;
				$msg="尊敬的".$arr[0]['name']."用户,您已被取消".$school."5、平安通知管理员的资格,如有疑问，请联系".$school."管理员";
				$data = '{
                    "touser":"'.$open.'",
                    "msgtype":"text",
                    "text":
	...
	public function actionOutschool(){
		...
		$con['sid']=$sid;
		//$m->where($con)->delete();
		WpIschoolUserRole::deleteAll($con);
