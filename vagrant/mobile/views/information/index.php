...
function addOneChild(){
  if(checkChildInfo()){
    ...
    var options = {para:data,ele:$("#add_span"),sub_url:url,to_url:to_url,urltype:0,status:[{code:0,content:'学校或班级填写错误'},{code:1,content:'学生信息尚未导入，请关闭当前页面，然后点击【我的服务】-》【人工客服】联系人工客服'},{code:2,content:'保存失败'},{code:3,content:'您已关注过该学生'}]};
    sub_dialog(options);
...
function saveclassinfo(){
    if(checkImginfo()){
        ...
        var sid=$.trim($("#school").val());
        var cid= $.trim($("#class").val());
        var url =path+"/information/saveclassinfo?sid="+sid+"&cid="+cid;
        loadHtmlByUrl(url); 
    }
}
...
function addOneClass(){
  ...
  var url=path+"/information/doaddclass";
    ...
    var options = {para:data,ele:$("#add_span"),sub_url:url,to_url:to_url,urltype:0,status:[{code:1,content:'学校不存在'},{code:2,content:'角色名重复，请勿重复申请同一角色'}]};
    sub_dialog(options);
  }
}
...
function removeOneChild(stuid){
  ...
  var d = dialog({
    title: '提示',
    content: '您确定要取消关注吗?取消关注后将收不到孩子的相关信息',
    okValue: '确定',
...
function savelxr(){
  if(checklxr())
  {
    ...
    $.post(url,{tel:tel,username:username,email:email,stuid:stuid},
       function (data){
         if(data==1){
    var d = dialog({
    title: '添加成功',
