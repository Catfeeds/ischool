;(function(RCS){
	...
	var SaveSendMessage = function(message){
		...
		var sendtime=message.sentTime;
                var sendId=message.senderUserId;
		var info = 'messageType='+messageType+'&converType='+converType+'&targetId='+targetId+'&sendtime='+sendtime+'&sendId='+sendId;	//将请求信息组成请求字符串
		if(messageType=="ImageMessage"){
			var imageUri=message.content.imageUri;
			info +='&imageUri='+escape(imageUri);
		}else if(messageType=="FileMessage"){
                        var fileUri=message.content.fileUrl;
			info +='&fileUri='+escape(fileUri);
	...
	var ajax =function(url,info,callbacks){
		//1.创建AJAX对象
	...
	var createIMConversation = function(config){
		...
		getParlist(callback,config.par);
		// getConversationList(callback);
	}

	    //获取会话列表
	var getParlist = function(callback,list){ 
               console.log(list);
	...
	var startConversation = function(event){
		...
		conversation.mcount = event.currentTarget.getAttribute('_mcount');
                conversation.name = event.currentTarget.getAttribute('_uname');
	...
	var openConversation = function(conversation){
		...
		var callbacks = function(list,hasMsg){
			...
			data.targetName = '用户：'+conversation.name+'('+conversation.id+')';
