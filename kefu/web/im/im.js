;(function(RCS){
	...
	//发送消息保存到本地
	var SaveSendMessage = function(message){
		var url="/kefu/savesendmessage";

		var messageType=message.messageType;
		
		var converType=message.conversationType;
		var targetId=message.targetId;
		var sendtime=message.sentTime;
		var info = 'messageType='+messageType+'&converType='+converType+'&targetId='+targetId+'&sendtime='+sendtime;	//将请求信息组成请求字符串
		if(messageType=="ImageMessage"){
			var imageUri=message.content.imageUri;
			info +='&imageUri='+escape(imageUri);
		}else{
	...
	var ajax =function(url,info,callbacks=null){
		//1.创建AJAX对象
		...
		ajax.onreadystatechange = function(){
			...
			if(ajax.readyState == 4 && ajax.status == 200){
				...
				if(msg!='success'){
					...
					if(callbacks){
						// console.log('145'+JSON.stringify(list));
						callbacks(list,hasMsg=false);
					}
				}else{
					console.log(msg);
				}
	...
	var updateMessage = function(message){
		...
		if(JSON.stringify(message.content.messageName)=='TypingStatusMessage'){
			return;
		}
	...
	//获取未读消息
	var getUnreadMessageCount = function(list){
		var conversationType = RongIMLib.ConversationType.PRIVATE;
		console.log('消息长度：'+list.length);	
		for (var i = 0; i < list.length; i++) {
			RongIMLib.RongIMClient.getInstance().getUnreadCount(conversationType,list[i].targetId,{
			    onSuccess:function(count){
			        // count => 指定会话的总未读数。
			        console.log('未读消息数1：'+count);			   
			    },
			    onError:function(){
			        // error => 获取指定会话未读数错误码。
			    }
	...
	//获取本地会话列表
	var getBendiConverList = function(callback){
		var url="/kefu/getcoverlist";
		var info='data=1';
		//1.创建AJAX对象
		var ajax = new XMLHttpRequest();
		  			...
		  			if(list.length>=1){
	            		...
	            		for (var i = 0; i < list.length; i++) {
				    		if (list[i].conversationType == RongIMLib.ConversationType.PRIVATE) {
				    			// getUnreadMessageCount(list);
				    			// list[i].unreadMessageCount=1;
				    			RongIMLib.RongIMClient.getInstance().getUnreadCount(conversationType,list[i].targetId,{
								    onSuccess:function(count){
								...
								// list[i].unreadMessageCount=BB[i];
				    			_list.push(list[i]);
				    		}
				    	}
				    	console.log(BB);
				    	console.log('123'+JSON.stringify(_list));
				    	var temp = _list[0];
				    	...
				    	for (var i = 0; i < list.length; i++) {
				    		for (var j = i+1; j < list.length; j++) {
				    			if (list[i].sentTime < list[j].sentTime) {
				    				var temp = list[i];
				    				list[i] = list[j];
				    				list[j] = temp;
				    			}
	            		...
	            		console.log('234'+JSON.stringify(list));
	...
	var openConversation = function(conversation){
		...
		getHistoryMes(conversation.id,callbacks);
	}
	//获取本地历史消息记录
	var getHistoryMes = function(conversationId,callbacks){
		var targetId = conversationId; // 想获取自己和谁的历史消息，targetId 赋值为对方的 Id。
		var url = "/kefu/gethismes";
		var info='targetId='+targetId;
		ajax(url,info,callbacks);
	}
	//拉去消息记录
	...
	//接收消息保存到本地
	var SaveRecMessage = function(message){
		var url="/kefu/saverecmessage";
		var messageType=message.messageType;
		var converType=message.conversationType;
		var targetId=message.targetId;
		var sendtime=message.sentTime;
		var info = 'messageType='+messageType+'&content='+content+'&converType='+converType+'&targetId='+targetId+'&sendtime='+sendtime;	//将请求信息组成请求字符串
		if(messageType=="ImageMessage"){
			var imageUri=message.content.imageUri;
			info +='&imageUri='+escape(imageUri);
		}else{
		...
		RongIMClient.setOnReceiveMessageListener({
			...
			onReceived: function (message) {
	            ...
	            if (message.conversationType == RongIMLib.ConversationType.PRIVATE) {
	            	...
	            	// console.log('image'+JSON.stringify(message));
	            	if(SaveRecMessage(message)){
	            		updateConversationList();
	            	};
