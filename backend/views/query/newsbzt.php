...
<?php if(!empty($bzc)){foreach($bzc as $key=>$value){ ?>
    <tr style="cursor:pointer;" class="alert alert-warning" data-toggle="collapse" data-target="#demob<?=$key?>"><td><?=$value['name'];?></td><td ><?=$value['id'];?></td><td>不正常</td></tr>
    <tr id="demob<?=$key?>"><td class="text-danger" colspan="3"><b><?php $bzcssxx = json_decode($value['pinganid'],true); foreach($bzcssxx as $k=>$v){?><?=$k.' / '?><?php } ?></b></td>
        </tr>
<?php }} ?>

    <?php if(!empty($zc['bfzc'])){ foreach($zc['bfzc'] as $key=>$value){ ?>
        <tr style="cursor:pointer;" class="alert alert-warning" data-toggle="collapse" data-target="#demo<?=$key?>">
            <td ><?=$value['sname'];?></td>
            <td ><?=$value['sid'];?></td>
            <td >部分正常</td>
        </tr> 
        <tr id="demo<?=$key?>">
        <td  colspan="3"><b class="text-danger"><?php foreach($value['pingan_bzcid'] as $k=>$v){?><?=$k.' / '?><?php } ?></b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b class="text-success"><?php foreach($value['pingan_zcid'] as $k=>$v){?><?=$k.' / '?><?php } ?></b>
        </td>
        </tr>
    <?php }} ?>
...
<?php if(!empty($zc['zc'])){foreach($zc['zc'] as $key=>$value){ ?>
    <tr class="success" style="cursor:pointer;" data-toggle="collapse" data-target="#demozc<?=$key?>">
        <td><?=$value['sname'];?></td><td><?=$value['sid'];?></td><td>正常</td></tr>
    <tr id="demozc<?=$key?>">
        <td class="text-danger" colspan="3"><b class="text-success"><?php foreach($value['pingan_zcid'] as $k=>$v){?><?=$k.' / '?><?php } ?></b></td>
    </tr>
<?php }} ?>
