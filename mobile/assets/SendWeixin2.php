...
class sendWeiXin2 {
    ...
    static $temp_id_2 = "IoqZusggp1A7kDOr6_8awRn2AJ-ZaFAar7kYdx7uf3c";
    ...
    static function getAccessToken() {
        ...
        }else if($schoolid=='100000'){
            $appId = "wx5c4d151389b92441";
            $appSecret = "d570c3cfa8143a398a4609ecc9427266";
        }else{
        ...
        if ($result) {
            if($schoolid==56650){
                ...
                $acc_token = $redis->get("my_access_token_qiye");
                if (!$acc_token) {
                    $json = file_get_contents($token_url);
                    $result = json_decode($json);
                    $acc_token = $result->access_token;
                    $redis->set("my_access_token_qiye",$acc_token, 7100);
                }
            }else{
    ...
    static public function sendSafe($to, $stuname, $msg, $timeinfo, $picurl,$sid) {
        $result = json_decode('{"errcode":0}');
        if (!empty($to)) {
           // $inoutDate = date("Y年m月d日H时i分s秒");
            global $schoolid;
            $schoolid=$sid;
            $title = "学生考勤信息";
            $content = "您的学生：" . $stuname . "于" . $timeinfo . "有一条" . $msg . "信息。\\n\\t\\n点击【家校互动】->【平安通知】查看学生进出校信息";
            $pub_num="正梵智慧校园";
            if($schoolid=='100000'){
                $title = "考勤信息";
                $content = "员工：" . $stuname . "于" . $timeinfo . "有一条" . $msg . "信息。\\n\\t\\n点击查看员工进出信息";               
                $pub_num="正梵企业管理";
            }
            ...
            $data = self::createTempMsg($to, $tempId, $title, $content,$picurl,$pub_num);
