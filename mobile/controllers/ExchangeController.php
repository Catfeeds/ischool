...
//                          var_dump( $list_level);die;
              ...
              $list_lxr=WpIschoolTeaclass::find()->select('id,tname')->where(['sid'=>$sid,'ispass'=>'y'])->groupBy('openid')->orderBy('convert(tname using gbk) asc')->asArray()->all();
...
//              var_dump($return_arr['list_level'][0]['classes']);die;
            ...
            $school = WpIschoolSchool::find()->select('name')->where(['id'=>$sid])->asArray()->all();
            if($school){
...
//            var_dump($return_arr);die;
     ...
     //执行发消息的方法
      public function actionDosendmsg(){  
         ...
         $result = $this->sendExchangeMsg($openid,$tos,$data,$sid);
      ...
      public function getOpidByLevels($sid,$levels){
         ...
         if(count($levelArr)=='1')  $levelArr=[$levels];
    ...
    public function getOpidByTids($tids,$operate_type){
         ...
         if($operate_type=='back'){
             ...
             $con[] = "and";
             $con[] = array('in','id',$tids);
     
             $res =WpIschoolTeaclass::find()->select('openid')->where($con)->asArray()->all();
//           $res =WpIschoolTeacher::find()->select('openid')->where(['in','id',$tids])->asArray()->all();         
     ...
     private function sendExchangeMsg($from,$tos,$data,$sid){
         $des     = $data['des'];        //纯内容
     ...
     function actionSearchlxr(){
        ...
        if(!empty($name)){
            // $sql = "select distinct id,tname from wp_ischool_teaclass where sid=".$sid." and tname like'%".$name."%' order by convert(tname using gbk) asc";
            $m=WpIschoolTeaclass::find()->select('id,tname')->where(['sid'=>$sid,'ispass'=>'y'])->andWhere(['like','tname',$name])->groupBy('openid')->orderBy('convert(tname using gbk) asc')->asArray()->all();
        }else{
            // $sql="select distinct id,tname from wp_ischool_teaclass where sid=".$sid." order by convert(tname using gbk) asc";
            $m=WpIschoolTeaclass::find()->select('id,tname')->where(['sid'=>$sid,'ispass'=>'y'])->groupBy('openid')->orderBy('convert(tname using gbk) asc')->asArray()->all();
        }
        // $m=WpIschoolTeaclass::findBySql($sql)->asArray()->all();
