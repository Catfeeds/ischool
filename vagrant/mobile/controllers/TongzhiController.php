...
//        $cids = D()->table("wp_ischool_pastudent as a left join wp_ischool_student as b on a.stu_id=b.id")->where($con)->field('a.stu_id,a.cid,a.class,a.stu_name,b.enddatejx')->select();
        return $cids;
    }
    /*  抓取本人所带班级 特定于老师角色 */
...
//        $sql = "select distinct t.cid,t.class from wp_ischool_teaclass t left join wp_ischool_class t2 on t2.id=t.cid where t.openid='".$openid."' and t.ispass='y' and t2.level !=0";
      $sql = "select distinct t.cid,t.class from wp_ischool_teaclass t left join wp_ischool_class t2 on t2.id=t.cid where t.openid=:openid and t.sid='".$sid."' and t.ispass='y' and t2.level !=0";
        return  WpIschoolTeaclass::findBySql($sql,[':openid'=>$openid])->asArray()->all();       
    }
    ...
    function getAlltea($openid,$sid){
        ...
        $cid = trim($cid,",");       
      if($cid==''){
           $sql = "select cid,id,tname,openid from wp_ischool_teaclass where cid in('".$cid."') and ispass='y' order by convert(tname using gbk) asc";
      }else{
          $sql = "select cid,id,tname,openid from wp_ischool_teaclass where cid in(".$cid.") and ispass='y' order by convert(tname using gbk) asc";
      }
    ...
    public function getAllStuByOpid($openid,$sid){
        ...
        $cid  = trim($cid,",");
        $sqls = "select id,name from wp_ischool_student where cid in(".$cid.") order by convert(name using gbk) asc";
        return  WpIschoolStudent::findBySql($sqls)->asArray()->all(); 
    }
     /*  抓相关的所有班级 */
