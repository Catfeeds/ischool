...
//        }
    ...
    public function actionDelchild(){
        ...
        if($stuid == $this->pauser[0]['last_stuid']){
            // echo "<script>alert('当前学生不能取消绑定！');</script>";
        ...
        if(!empty($res)){
            ...
            if($painfo){                        //家长表中存在有孩子信息
                ...
                if(!empty($popenid)){       //家长表中有有孩子信息和openid
                    ...
                    $uppastu->uid = $this->getUid($openid);
...
//添加亲情号码
    public  function actionAddqqh(){
        ...
        $post = \yii::$app->request->post();
        $model =new Pastudent();
        if(in_array($post['sid'],$arrsid)){
            $res = $model::find()->where(['stu_id'=>$post['stuid'],'ispass'=>'y','isqqtel'=>1])->asArray()->all();
            if(count($res) ==3){
                echo "<script>alert('您好，您添加亲情号码的个数已经超过限制，最多只能添加三个亲情号码！');window.location='/pastudent/child';</script>";
                exit();
            }
        }
...
//请假申请
    ...
    public function actionDoscorequery(){
        ...
        $isopen = $post['isopen'];
        if($stuid != "all"){   //查个人
            $sql = "SELECT c.stuid,c.stuname,c.kmname,c.score,t.sort FROM wp_ischool_chengji c left join wp_ischool_chengji_kemu t on c.kmid=t.id WHERE c.cid=:cid AND c.cjdid=:cjdid AND c.stuid=:stuid order by t.sort asc";
            $users = \Yii::$app->db->createCommand($sql,[':cid'=>$cid,':cjdid'=>$cjdid,':stuid'=>$stuid])->queryAll();
        }elseif($isopen == "y" && $stuid == "all"){
            $sql = "SELECT c.stuid,c.stuname,c.kmname,c.score,t.sort FROM wp_ischool_chengji c left join wp_ischool_chengji_kemu t on c.kmid=t.id WHERE c.cid=:cid AND c.cjdid=:cjdid order by c.stuid asc,t.sort asc"; 
...
//发件箱删除信息
    ...
    public function actionInbox(){
        ...
        $outbox = \Yii::$app->db->createCommand($sql,[':content'=>$content,':outopenid'=>$paopenid,':ctime'=>$ctime,':title'=>$title,':out_uid'=>$out_uid]);
        $outbox = $outbox->execute();
        $transaction = \Yii::$app->db->beginTransaction();       //事务开始
        try{
            foreach($toopenid as $inopenid){
                $in_uid=$this->getUid($inopenid);
                $sql = "INSERT INTO `wp_ischool_inbox` ( `content` , `outopenid` , `inopenid` , `ctime` , `title` , `type` , `out_uid` , `in_uid` ) VALUES( :content, :outopenid,:inopenid, :ctime,:title,0,:out_uid,:in_uid)";
                $inbox = \Yii::$app->db->createCommand($sql,[':content'=>$content,':outopenid'=>$paopenid,':inopenid'=>$inopenid,':ctime'=>$ctime,':title'=>$totitle,':out_uid'=>$out_uid,':in_uid'=>$in_uid])->execute();
            }
            $transaction->commit();
...
//            var_dump($info['is_time']);exit();
    ...
    public  function actionRecords(){
        ...
        if(!empty($stuinfo))
        {
            ...
            if(in_array($sid,$jinmai_school)){
                if($get['type']=='xiexin'){
                    $beginTime=date("Ymd",$this->getBeginTimestamp('today'));
                }elseif($get['type']=='shouxin'){
                    $beginTime=date("Ymd",$this->getBeginTimestamp('week'));
                }elseif($get['type']=='yifa'){
                    $beginTime=date("Ymd",$this->getBeginTimestamp('month'));
                }
                ...
                $requst_info=[
                    ...
                    'serialType'=>'1',
                    'serialValue'=>$stuno2,
                    'beginTime'=>preg_replace('/[-]/','', $beginTime),
                ...
                if($jsonInfo['resultCode']=='0000' && empty($jsonInfo['datas'])){
                    ...
                    foreach($jsonInfo['datas']  as $k=>$v){
                        $result[$k]['created']=strtotime(substr($v['tradeTime'],0,4)."-".substr($v['tradeTime'],4,2)."-".substr($v['tradeTime'],6,2).substr($v['tradeTime'],8,2).":".substr($v['tradeTime'],10,2).":".substr($v['tradeTime'],12,2));                       
                        $result[$k]['pos_sn']=$v['tradeAdd'];
                    ...
                    $pagination = new Pagination([
                        'defaultPageSize' => 9,
                        'totalCount' =>count($jsonInfo['datas']),
                    ]);

                    if($get['type']=='xiexin'){
                        $info['today']['model']=$result;
                        $info['today']['pages']=$pagination;
                    }elseif($get['type']=='shouxin'){
                        $info['week']['model']=$result;
                        $info['week']['pages']=$pagination;
                    }elseif($get['type']=='yifa'){
                        $info['month']['model']=$result;
                        $info['month']['pages']=$pagination;
                    }
                }
            }else{
                ...
                $begintm = $this->getBeginTimestamp('today');       //获得今天的时间戳
                $info["today"] = $this->getxflist($stuno2,$begintm,$sid);

                $begintmwk = $this->getBeginTimestamp("week");       //获得本周的时间戳
                $info['week'] = $this->getxflist($stuno2,$begintmwk,$sid);
                $begintmmt = $this->getBeginTimestamp("month");       //获得本月的时间戳
                $info['month'] = $this->getxflist($stuno2,$begintmmt,$sid);
            }
    ...
    //获取消费列表信息
    public function getxflist($stuno,$begintm,$sid){
        
        // echo $stuno;die;
        $cardinfo = ZfCardInfo::find()->where(['user_no'=>$stuno,"school_id"=>$sid])->select('card_no')->asArray()->all();
        $card_no = $cardinfo[0]['card_no']; //card表中学生学号user_no
        $models = ZfDealDetail::find()->where(['card_no'=>$card_no,"school_id"=>$sid])->andFilterWhere(['>','created',$begintm]);
        $pagination = new Pagination([
            'defaultPageSize' => 10,
