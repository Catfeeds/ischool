...
//本地订单处理逻辑
$sql = "select id from wp_ischool_orderbk where stuid='".$result[3]."' and trade_no='" .$result[4]. "' and ispass=0";
$resu=$pdo->query($sql);
if($resu){
        $pee = $result[5];
         $utime = time();
        $up_order_sql = "update wp_ischool_orderbk set ispass=1,utime=".$utime.
            ",zfopenid='".$result[8]."',trans_id=".$result[7]." where stuid='".$result[3].
            "' and trade_no=".$result[4]." and ispass=0";
        $up_order_sql =$pdo->exec($up_order_sql);
        // $syx = $mysql->affected_rows; //记录影响行数
        $openid = $result[6];
        if($up_order_sql){
            $content="补卡功能费提醒!"."您已为学生".$result[2]."提交补卡缴费".$result[5]."元，谢谢您的使用！";
            $titie="充值成功";
            $data = '{
                        "touser":"'.$openid.'",
                        "msgtype":"text",
         ...
         if($result[1]==56650){
             ...
             // SendMsgs::sendSHMsgToPa($openid,$titie,$content,$url="",$picurl="");
