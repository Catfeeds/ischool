...
class PayController extends BaseActiveController
{
	...
	/*
        ...
        $input->SetNotify_url("http://api.jxqwt.cn/pay/wxpaynotify");
	...
	*/
	...
	$result = $app->order->unify([
    	...
    	'total_fee' => 88,
    	'notify_url' => 'http://api.jxqwt.cn/pay/wxpaynotify', // 支付结果通知网址，如果不设置则会使用配置里的默认地址
    	'trade_type' => 'APP',
	]);
	var_dump($result);
    ...
    public function actionWxpaysk()
    {
        ...
        $pay_subject = $xingming.'-'.$this->post['title'];
...
//	$pay_body = "测试数据";
	...
	$bizcontent = "{\"body\":\"$pay_body\"," 
                ...
                . "\"product_code\":\"QUICK_MSECURITY_PAY\""
                . "}";
        $request->setNotifyUrl("http://api.jxqwt.cn/pay/alipaynotify");
...
//        return $this->formatAsjson($payinfo);
    ...
    public function actionSolution(){
        ...
        $bizcontent = "{\"body\":\"$pay_body\","
            ...
            . "\"product_code\":\"QUICK_MSECURITY_PAY\""
            . "}";
        $request->setNotifyUrl("http://api.jxqwt.cn/pay/alipaynotifyck");
        $request->setBizContent($bizcontent);
        //这里和普通的接口调用不同，使用的是sdkExecute
        $response = $aop->sdkExecute($request);
        if ($type ==="canka"){
    ...
    public function actionAlipaynotifyck()
    {
        ...
        if($type ==="canka"){
            ...
            if($resu){
                \Yii::trace(111111);
                $up_rechange_sql="insert into zf_recharge_detail (card_no,credit,type,balance,pos_no,created_by,time,note,is_active,school_id,trade_no) values (:card_no,:credit,'ZFBJSAPI','0','0',:created_by,:time,'0','0',:school_id,:trade_no)";
                $stmt2=\Yii::$app->db2->createCommand($up_rechange_sql,[":card_no"=>$resu[0]['card_no'],":credit"=>$money,":created_by"=>$resu[0]['created_by'],":time"=>time(),":school_id"=>$sid,":trade_no"=>$trade_no])->execute();
            }
        }elseif ($type !=="canka"){
            $resus=WpIschoolOrderjf::findOne(['trade_no'=>$trade_no]);
    ...
    public function actionWater(){
        ...
        $bizcontent = "{\"body\":\"$pay_body\","
            ...
            . "\"product_code\":\"QUICK_MSECURITY_PAY\""
            . "}";
        $request->setNotifyUrl("http://api.jxqwt.cn/pay/alipaynotifywt");
        $request->setBizContent($bizcontent);
        //这里和普通的接口调用不同，使用的是sdkExecute
        $response = $aop->sdkExecute($request);
        if ($type ==="water"){
    ...
    public function actionWxpayrbk(){
        $params = $this->post;
        // empty($params['amount_adult']) || empty($params['amount_stu']) ||
        if (empty($params['sid']) || empty($params['time']) ||  empty($params['type']) || empty($params['uid']) || empty($params['money']) || empty($params['stu_id'])  || empty($params['title'])) {
                return $this->errorHandler("1061");
            }
        if (time()>strtotime($params['time'])) {
            return $this->errorHandler("1063");
        }
        if (strtotime($params['time'])>1535731200) {
            return $this->errorHandler("1064");
        }
        ...
        $input->SetTime_start(date("YmdHis"));
        $input->SetTime_expire(date("YmdHis", time() + 600));
        $input->SetNotify_url("http://api.jxqwt.cn/pay/wxpaynotifyrbk");
        $input->SetTrade_type("APP");
        $result = \WxPayApi::unifiedOrder($input);
        \Yii::trace($result);
        // Log::DEBUG("unifiedorder:" . json_encode($result));
    ...
    public function actionAlipayrbk(){
        ...
        $bizcontent = "{\"body\":\"$pay_body\"," 
                    ...
                    . "\"product_code\":\"QUICK_MSECURITY_PAY\""
                    . "}";
        $request->setNotifyUrl("http://api.jxqwt.cn/pay/alipaynotifyrbk");
        $request->setBizContent($bizcontent);
        //这里和普通的接口调用不同，使用的是sdkExecute
        $response = $aop->sdkExecute($request);
        // 插入数据库订单  
...
// 基类的方法支付方法需要重写
        ...
        $notify->Handle(true);
        \Yii::trace($data);
        $params['trans_id'] = $data['transaction_id']; //微信交易号
        $params['trade_no'] = $data['out_trade_no'];
...
//瑞贝卡支付宝支付回调
    public function actionAlipaynotifyrbk(){
        // $_post['body']
        \Yii::trace($_POST);
        $params['trans_id'] = $_POST['trade_no']; //支付宝交易号
        $params['trade_no'] = $_POST['out_trade_no'];
    ...
    public function Total_hs($params){
        ...
        $time = $params['time'];
        $amount_adult = !empty($params['amount_adult'])?$params['amount_adult']:0;
        $amount_stu = !empty($params['amount_stu'])?$params['amount_stu']:0;
        $type = $params['type'];
        \Yii::trace($amount_stu);\Yii::trace($amount_adult);
        $inweek = date('w',strtotime($time));
        $week_one = [1,2,3,4];
        $week_two = [5,6,0];
        if($type == 'ssj'){                     //水世界
            if(in_array($inweek,$week_one)){
                $amount = $amount_adult*70 + $amount_stu*36;
            }elseif (in_array($inweek,$week_two)) {
                $amount = $amount_adult*80 + $amount_stu*58;
            }
        }elseif ($type == "skwg") {      //水卡王国
            if(in_array($inweek,$week_one)){
                \Yii::trace(1111);
                $amount = $amount_adult*60 + $amount_stu*60;
            }elseif (in_array($inweek,$week_two)) {
                \Yii::trace(2222);
                $amount = $amount_adult*60 + $amount_stu*60;
            }
