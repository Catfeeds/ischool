...
use mobile\controllers\UtilsController;
use mobile\models\WpIschoolSafecard;
use mobile\models\WpIschoolAdminer;
use mobile\models\WpIschoolSuggest;
...
class InformationController extends BaseController {
    ...
    public function actionIndex(){              
        ...
        {
            (\yii::$app->request->get("status") && \yii::$app->request->get("status") == "1") ? $status=1 :  $status=0;                  
            if(($res[0]['last_sid']==56758 || $res[0]['last_sid']==56650) && $status==0 ){                        
                    $url = Url::toRoute(['homepage/index','openid'=>$openid,'sid'=>$res[0]['last_sid']]);
                    return $this->redirect($url);
    ...
    public function actionSaveuserinfo(){
        ...
        $res=WpIschoolUser::find()->where(['tel'=>$tel])->one();
         // var_dump($res->tel);die;
        if(empty($res)){
            ...
            $m->save(false);
    	    $title="来自智慧校园的消息";
    	    $des = "尊敬的用户您好，您的初始登录密码为".$info."，用此密码您可以在电脑端http://www.jxqwt.cn/,选择对应的身份进行登录，感谢您的注册。";
            $session = \Yii::$app->session;
            $school_id=  $session['school_id'];        
            if($school_id==56650){
                SendMsg::sendSHMsgToPa($openid,$title,$des,$url="http://mobile.jxqwt.cn/information/index?openid=".$openid."&sid=56650",$picurl="");  
            }else{
                SendMsg::sendSHMsgToPa($openid,$title,$des,$url="",$picurl=""); 
            }
            $at="success";
        }
        else
        {
            if(empty($res->openid)){
                $res->openid=$openid;
                $res->save(false);
                echo "<script>window.location.href='/information/index'</script>";
            }else{
                $at=0;
...
//       var_dump($par);die;
     ...
     public function actionAddonechild(){
        ...
        // $sql='select distinct pro from wp_ischool_school ORDER BY convert(pro USING utf8)';
        $sql='select distinct pro from wp_ischool_school';
...
//         $sql="select id,name from wp_ischool_school where schtype='".$cname."' and county='".$area."' ORDER BY convert(name USING gbk)";
        $sql="select id,name from wp_ischool_school where county='".$area."' ORDER BY convert(name USING gbk)";
        $res=WpIschoolSchool::findBySql($sql)->asArray()->all();
        ...
        if($res) {
            ...
            else
            {
                ...
                if($has){
                    ...
                    $mm->uid  = $this->getUid();
                    $idp = $mm->save(false);
		    $last_stuid = \Yii::$app->db->getLastInsertID();
                    if($idp > 0 || $idp === 0)
                    {
                        $at = "success";
                        $where = "";
                        ...
                        $title="来自智慧校园的消息";
                        $des = "您已成功绑定学生".$student."。您可以点击当前消息，了解相关业务，并根据自己的需要开通使用。";
                        //学校、班级、姓名、sid、stu_id、openid、stuno2
                      
                        $tc=$school."|".$classname."|".$student."|".$sid."|".$stuid."|".$openid."|".$res[0]['stuno2'];
                        $session = \Yii::$app->session;
                        $school_id=  $session['school_id'];        
                        if($school_id==56650){
                            SendMsg::sendSHMsgToPa($openid,$title,$des,$url="http://mobile.jxqwt.cn/zfend/recharge?tc=".$tc."&sid=56650",$picurl="");  
                        }else{
                            SendMsg::sendSHMsgToPa($openid,$title,$des,$url="http://mobile.jxqwt.cn/zfend/recharge?tc=".$tc."&sid=$sid",$picurl=""); 
                        }
                        $at = 5;
                    }
                    else
...
//        $sql="select * from wp_ischool_pastudent where isqqtel=1 and stu_id=$stu_id and is_deleted= 0 and tel IS NOT NULL";
        $sql="select * from wp_ischool_pastudent where stu_id=$stu_id and is_deleted= 0 and tel IS NOT NULL";
     ...
     //变更学生卡号，EPC号和学生电话号信息
     public function actionEditepc(){
         $stu_id=\yii::$app->request->get("id");
         $stuinfo =WpIschoolStudent::find()->where(['id'=>$stu_id])->asArray()->all();
         $return_arr['stuinfo']=$stuinfo; 
         $return_arr['stuid']= $stu_id;
//       var_dump($stuinfo);die;
         return $this->render('editepc',$return_arr); 
     }
     //更新epc，电话卡和学生卡号
     public function actionUpdateepc(){
         //获取到的卡号
         $kahao=\yii::$app->request->get("name");
        //学生id 
         $stu_id=\yii::$app->request->get("stuid");
         $info=WpIschoolKaku::find()->where(['stuno2'=> $kahao])->asArray()->one();
...
//          var_dump($result);die;
    ...
    public function actionUpdateqqh(){
     ...
     public function actionAskchildleave(){
        ...
        $sname = $student[0]['name'];
         $sid = $this->getSchoolsid($stuid)[0]['sid'];
        if($leave === 0 || $leave > 0){
            $result = 0;
            $this->doSendLeave($topenid,$sname,$sid);
        }else{
            $result = 1;
        }
        $this->ajaxReturn($result,"json");  
    ...
    private function doSendLeave($tos,$sname,$sid){
        ...
        foreach ($tos as $v) {
            $final = SendMsg::sendSHMsgToPa($v['openid'], $title, $sname . $msg,"",$picurl);
        }
        return $final;
    }
      /**
    ...
    public function actionDeleonechild(){
        ...
        $userinfo = WpIschoolUser::findOne(['openid'=>$openid]);
       
        $last_stuid = $userinfo['last_stuid'];
       
        if($stu_id == $last_stuid){
            $userinfo->last_stuid =Null;
            $userinfo->save(false);
        }
        $count=WpIschoolPastudent::find()->where(['openid'=>$openid])->count();
        if($count<=1){
            $this->ajaxReturn('refuse','json');
        }
    ...
    public function actionP_changeschool(){
        $stu_id=\yii::$app->request->post("stuid");
        $openid=\yii::$app->request->post("openid");
        $arr=WpIschoolStudent::find()->where(['id'=>$stu_id])->asArray()->one();            
        // $data=  WpIschoolSchool::find()->select('id')->where(['name'=>$arr[0]['school']])->asArray()->all(); 
        $user= WpIschoolUser::find()->where(['openid'=>$openid])->one();     
        $user->last_sid=$arr['sid'];
        $user->last_cid=$arr['cid'];
        $user->last_stuid=$arr['id'];
        $user->save(false);      
        $sid=$arr['sid'];
        if($user==1){
            $at["sid"]=$sid;
     ...
     /* 教师执行学校切换操作 成功后由页面执行刷新 */
    public function actionT_changeschool(){
        $sid=\yii::$app->request->post("sid");
        $openid=\yii::$app->request->post("openid");
        $user= WpIschoolUser::find()->where(['openid'=>$openid])->one();
        $user->last_sid=$sid;
    ...
    public function actionMyallclass(){
        ...
        $where="";
        $where['openid']=$openid;
         $res = WpIschoolTeaclass::find()->where($where)->groupBy(['role'])->asArray()->all();
        if(empty($res))
        {
            //没有绑定教师信息
            $res=1;
        ...
        // var_dump($return_arr['list_class']);die;
        $lastsid= WpIschoolUser::find()->select('last_sid')->where(['openid'=>$openid])->asArray()->all();
        $return_arr['sid']=$lastsid[0]['last_sid'];
    ...
    public function actionAddoneclass(){
     ...
     public function actionDoaddclass(){
        ...
        $tel=$res[0]["tel"];
        $tname=$res[0]["name"];
        //查询老师班级表，如果该班级已经存在对应职位老师，则提示请联系管理员
        $wh['school']=$school;
        $wh['class']=$classname;
        $wh['role']=$role;
        $wh['ispass']='y';
        $teaClass = WpIschoolTeaclass::find()->select('class,role')->where($wh)->asArray()->all(); 
 ...
 //                 $m->save(false); 
	...
	// }
        ...
        if($isSendSHMsg){
            ...
            foreach ($er as $v) {
                SendMsg::sendSHMsgToPa($v["openid"],$title,$des,"",$data['picurl']);
            }
        }       
        $this->ajaxReturn($at,'json'); 
     }
     ...
     public function actionAllstudent(){
       ...
       if($res){
            ...
            foreach($res as $k=>$v){
                $time=time()-86399;
                $status=WpIschoolSafecard::find()->where(['stuid'=>$v['id']])->andWhere(['>','receivetime',$time])->orderBy('receivetime desc')->limit(1)->asArray()->one();              
                $res[$k]['status']=$status['info'];               
                
            }

            foreach($res as $k=>$v){
                if(!preg_match('/(进宿舍)/',$v['status'])){
                    unset($res[$k]['status']);                  
                }else{
                    $renshu++;   
                }
...
//        var_dump($stuid);die;
     ...
     */
     ...
     public function actionLeavestu(){
      ...
      public function actionQingjiainfo(){
        ...
        $name=\yii::$app->request->get("name");
        $openid=\yii::$app->request->get("openid");
        $res=WpIschoolStuLeave::find()->select('reason,begin_time,stop_time')->where(['id'=>$id])->asArray()->all();   
        $return_arr['content']=$res[0]["reason"];
        $return_arr['begin_time']=$res[0]["begin_time"];
        $return_arr['stop_time']=$res[0]["stop_time"];
        $return_arr['tcid']=$tcid;
        $return_arr['cid']=$cid;
        $return_arr['sid']=$sid;
     ...
     public function actionResponsestuleave(){
        ...
        //获取请假学生的姓名
        $res=WpIschoolStudent::find()->select('name')->where(['id'=>$stuid])->asArray()->all(); 
        $sname=$res[0]['name'];
        $sid = $this->getSchoolsid($stuid)[0]['sid'];
        ...
        if($flag == 0){   //批准
            ...
            if($res === 0 || $res > 0){
                $result = 0;
                $msg = $sname."家长，您发送的请假请求被拒绝。若有问题请重新申请或联系班主任";
                $this->doSendLeavep($popenid,$msg,$sid);
            }else{
                $result = 1;
            }
    ...
    public function doSendLeavep($info,$msg,$sid){
        ...
        $data['picurl'] =$this->getSchoolPics($sid);
        foreach ($info as $v) {
            $final = SendMsg::sendSHMsgToPa($v["openid"],"请假信息",$msg,"",$data['picurl']);
        }
        return $final;
    }
...
//                    return $this->assignPage("文件格式错误");
    ...
    //根据学生stuno2获取学校sid
    function getSchoolsidbystuno2($stuno2){
        $sid = WpIschoolStudent::find()->select('sid')->where(['stuno2'=>$stuno2])->asArray()->all();
        return $sid;
    }
    //根据学生stuno2获取学校sid
    function getSchoolsidbycid($cid){
        $sid = WpIschoolClass::find()->select('sid')->where(['id'=>$cid])->asArray()->all();
        return $sid;
    }
    ...
    private function getAllSysSubject(){
      ...
      public function actionDeleoneclass(){
	    ...
	    if(!WpIschoolTeaclass::findOne(['sid'=>$res[0]["sid"],"openid"=>$res[0]["openid"]])){
            ...
            $where["shenfen"]="tea";
            if($m= WpIschoolUser::find()->where($where)->one()){
                if($id == $res[0]['last_cid']){
                    $m->last_cid="";
                }
                $m->last_sid=1;
                $m->save(false);
...
//       var_dump($re);die;
    ...
    public function GiveEpcrole($pl,$user,$sid){
        ...
        if(empty($arr)){           
         ...
         $title="来自智慧校园的消息通知";
         $data['picurl'] =$this->getSchoolPics($sid);
         SendMsg::sendSHMsgToPa($user,$title,$des,"",$data['picurl']);
         // SendMsg::https_post(SendMsg::getUrl('kf'),$data);
         // ，您登录平安通知系统的用户名为：".$ar[0]['tel']."密码为：".$pwd
    }
    //扫码绑定班主任中转方法
    public function actionSmurl(){      
//        $appid = "wx8c6755d40004036d";
//        $secret = "22f68f4da5b36641ed492c596406b75f";
        $cid=\yii::$app->request->get("cid"); //班级ID
        $sid = $this->getSchoolsidbycid($cid)[0]['sid'];
        if($sid == '56650'){
            $appid = SGAPPID;
            $secret = SGAPPSECRET;
        }else{
        ...
        $url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$appid.'&redirect_uri='.URL_PATH.'/information/smaddclass?cid='.$cid.'&sid='.$sid.'&getcode&response_type=code&scope=snsapi_userinfo&state=123#wechat_redirect';
	return $this->redirect($url);
    }
...
//        $secret = "22f68f4da5b36641ed492c596406b75f";
        $cid=\yii::$app->request->get("cid"); //班级ID
        $code=\yii::$app->request->get("code"); //code信息
        ...
        if($isSendSHMsg) {
            ...
            $er = WpIschoolUserRole::find()->select('openid')->where($where)->asArray()->all();
            $data['picurl'] =$this->getSchoolPics($classinfo[0]['sid']);
            foreach ($er as $v) {
                SendMsg::sendSHMsgToPa($v["openid"], $title, $des,"",$data['picurl']);
            }
        }
    ...
    //扫码绑定家长中转方法
    public function actionSmjzurl2($stun=null){   
//        $appid = "wx8c6755d40004036d";
//        $secret = "22f68f4da5b36641ed492c596406b75f";     
        if(\yii::$app->request->get("stuno2")){
             $stuno2=\yii::$app->request->get("stuno2"); //学生编号 
        }else{
            $stuno2=$stun;
        }
        $sid = $this->getSchoolsidbystuno2($stuno2)[0]['sid'];
        if($sid == '56650'){
            $appid = SGAPPID;
            $secret = SGAPPSECRET;
        }else{
...
//        var_dump($appid);exit();
        $url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$appid.'&redirect_uri='.URL_PATH.'/information/dosmaddchild?stuno2='.$stuno2.'&sid='.$sid.'&getcode&response_type=code&scope=snsapi_userinfo&state=123#wechat_redirect';
...
//          $xx = $x->where('stuno2="'.$stuno2.'"')->select();
    ...
    public function actionSaveclassinfo(){
        ...
        $allstu=WpIschoolStudent::find()->select('name')->where(['in','cid',$cid])->asArray()->orderBy('convert(name using gbk) collate gbk_chinese_ci')->all();
        if(Yii::$app->request->isPost){
            ...
            $school=\yii::$app->request->post("school");
            $classname=\yii::$app->request->post("classname");
            $student=\yii::$app->request->post("stu_name");
            $upload='upload/photos/'.$school.'/'.$classname.'/';
            ...
            if ($model->file && $model->validate()) {  
                ...
                $filepath= $upload.$student. '.' . $model->file->extension;
                ...
                Image::thumbnail($filepath,319,425)->save(Yii::getAlias($filepath), ['quality' => 100]);
                            
                $picurl= "http://mobile.jxqwt.cn/".$filepath;                 
                $stu= WpIschoolStudent::find()->where(['sid'=>$sid,'cid'=>$cid,'name'=>$student])->one();
                if($stu){
                     $stu->avatar= $picurl;
                     $isdo =$stu->save(false);
                }
        ...
        $return_arr['cid']=$cid;
        // $return_arr['student']=$student;
        $return_arr['classname']=$classname;
        $return_arr['school']=$school;
        $res = parent::Jssdk();     
    ...
    public function actionCheckstuname(){
        ...
        $school=\yii::$app->request->post("school");
        $classname=\yii::$app->request->post("classname");
        $res=$this->isHasstudent($sname,$school,$classname);
        if($res){
            $at['status']='success';
        }else{
            $at['status']='fail';
...
//         $childDir = date("y/m/d");   
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($model->file && $model->validate()) {  
                ...
                $model->file->saveAs($filepath);                             
                Image::thumbnail($filepath,319,425)->save(Yii::getAlias($filepath), ['quality' => 100]);
                $new_path=\UpImage::movefile($filepath,$school,$classname);             
                $picurl= $filepath;    
                $m= new  WpIschoolClassImages;         
                $m->sid = $sid;
                $m->cid=$cid;
                $m->picurl = $filepath;
                $isdo = $m->save(false);
                $s= WpIschoolStudent::find()->where(['sid'=>$sid,'cid'=>$cid,'name'=>$student])->one();
                $s->avatar= $new_path;
                $isdo =$s->save(false);
                if($isdo){
                    echo "<script> alert('上传成功！');</script>";die;
     ...
     //超级管理员权限入口
     public function actionAdminner(){
        $sid=\yii::$app->request->get("sid"); 
        if(empty($sid)){
            $sid=\yii::$app->request->params("sid");
        }
        $openid=\yii::$app->request->get("openid");
        $list_school=WpIschoolAdminer::find()->where(['openid'=>$openid])->asArray()->all();
        $return_arr['list_school']=$list_school;
        $return_arr['sid']=$sid;
        $return_arr['openid']=$openid;
        return $this->render('adminner',$return_arr);
     }
     //超管的学校
     public function actionAdminSchool(){
        ...
        $return_arr['schname']=$rp[0]["name"];
        $return_arr['sid']=$sid;                   
         return $this->render('admin-school',$return_arr); 
     }
    //添加超管的学校
     public function actionAddAdminSchool(){
        ...
        $adminer=WpIschoolAdminer::find()->select('openid')->where(['openid'=>$openid,'sid'=>$sid])->asArray()->one();
        if(!$adminer){
            $user=WpIschoolUser::find()->where(['openid'=>$openid])->asArray()->one();
            $adm=new WpIschoolAdminer;
            $adm->name=$user['name'];
            $adm->openid=$openid;
            $adm->school_name=$res[0]['name'];
            $adm->sid=$sid;
            $adm->role=0;
            $adm->save(false);
//            $query="update  wp_ischool_adminer set role=0 where openid = '".$openid."' and sid != ".$sid;
//            $ok=Yii::$app->db->createCommand($query)->execute();
            
            $num=\yii::$app->view->params['sid'] ;
        }else{
            $num=0;
        }
        $this->ajaxReturn($num,'json');
     ...
     //切换学校校长身份
     public function actionChangePower(){
         $sid=\yii::$app->request->post('sid');
         $openid=\yii::$app->request->post('openid');
         $school_name=\yii::$app->request->post('school_name');
         $name=\yii::$app->request->post('name');
        //把adminer表role变了
         $query="update  wp_ischool_adminer set role=0 where openid = '".$openid."' and sid != '".$sid."'";
         $up1=Yii::$app->db->createCommand($query)->execute();
         $query="update  wp_ischool_adminer set role=1 where openid = '".$openid."' and sid = '".$sid."'";
         $up2=Yii::$app->db->createCommand($query)->execute();
         //user_role有没有相关学校的角色身份
         $result=WpIschoolUserRole::find()->where(['openid'=>$openid,'sid'=>$sid])->asArray()->one();
         //没有结果插入一条数据。同时把该用户在user_role表的其他数据删除
         if(!$result){
             $p=new WpIschoolUserRole;
             $p->openid=$openid;
             $p->rid=1;
             ...
             $teaclass=new WpIschoolTeaclass;
             $teaclass->tname=$mm["name"];
             $teaclass->openid=$openid;
             $teaclass->school=$school_name;
             $teaclass->sid=$sid;
             $teaclass->class="管理";
             $teaclass->cid=0;
             $teaclass->role="校长";
...
//             $this->GiveEpcrole($pl,$openid,$sid);
             ...
             //删除user_role表不是当前切换学校的信息
             $query="delete from wp_ischool_user_role where openid = '".$openid."' and sid != '".$sid."'";
             $del1=Yii::$app->db->createCommand($query)->execute();
             //删除teaclass表不是当前学校的信息
             $query="delete from wp_ischool_teaclass where openid = '".$openid."' and sid != ".$sid." and cid=0 and role='校长' and class='管理'";
             $del2=Yii::$app->db->createCommand($query)->execute();  
             $num['sid']=$sid;
             $num['openid']=$openid;
             $num['status']='success';
         }else{
     ...
     //删除当前的的adminner
     public function actionDeladminer(){
         $id=\yii::$app->request->get('id');
         $result=WpIschoolAdminer::find()->where(['id'=>$id])->asArray()->one();
        
         if($result['role']==1){           
             //是当前角色校长的身份，需要删除user_role和teaclass的信息
         
          $user_role=WpIschoolUserRole::find()->where(['openid'=>$result['openid'],'sid'=>$result['sid']])->one();
          $user_role!=null ? $user_role->delete() : $user_role=null;
          $query="delete from wp_ischool_teaclass where openid = '".$result['openid']."' and sid = ".$result['sid']." and class='管理' and role='校长'";
          $del2=Yii::$app->db->createCommand($query)->execute(); 
                    
         }
         $a=WpIschoolAdminer::deleteAll(['id'=>$id]);  
    ...
    public function actionDocument(){
        ...
        $sid=\yii::$app->view->params['sid'];
        $openid=\yii::$app->view->params['openid'];
        $user=WpIschoolUser::find()->select('name')->where(['openid'=>$openid])->asArray()->one();
        if($user){
           $return_arr['username']=$user['name']; 
        }
        $return_arr['sid']=$sid;
        $return_arr['openid']=$openid;
        $return_arr['qunzu']=$qunzu;
        return $this->render('document',$return_arr);
    }
    public function actionDocumentInbox(){
        $openid=\yii::$app->view->params['openid'];
        $qunzu=\yii::$app->request->get('qunzu');
        $sid=\yii::$app->view->params['sid'];
      
        $nm= Attachment::find()->where(['sid'=>$sid,'type'=>$qunzu])->asArray()->count();
       ...
       $num=5;
        ...
        $next=$page+1;
        $last=$page-1;  
        $return_arr['start']=$path."/information/document-inbox?qunzu=".$qunzu."&sid=".$sid."&page=1";
        $return_arr['up']=$path."/information/document-inbox?qunzu=".$qunzu."&sid=".$sid."&page=".$last;
        $return_arr['down']=$path."/information/document-inbox?qunzu=".$qunzu."&sid=".$sid."&page=".$next;
        $return_arr['end']=$path."/information/document-inbox?qunzu=".$qunzu."&sid=".$sid."&page=".$sum;
        
          
        $res=Attachment::find()->where(['sid'=>$sid,'type'=>$qunzu])->orderBy('create_time desc')->offset($star)->limit($num)->asArray()->all();
        ...
        $return_arr['qunzu']=$qunzu;
        $return_arr['sid']=$sid;
        return $this->render('document-inbox',$return_arr);
    }
    public function actionDocumentOutbox(){
        $openid=\yii::$app->view->params['openid'];
        $qunzu=\yii::$app->request->get('qunzu');
        $sid=\yii::$app->view->params['sid'];
        
        $nm= Attachment::find()->where(['sid'=>$sid,'type'=>$qunzu,'openid'=>$openid])->asArray()->count();
        $page=\yii::$app->request->get("page");
        //每页显示的条数
        $num=5;
        //一共多少页
        ...
        $next=$page+1;
        $last=$page-1;
        $return_arr['start']=$path."/information/document-outbox?qunzu=".$qunzu."&sid=".$sid."&page=1";
        $return_arr['up']=$path."/information/document-outbox?qunzu=".$qunzu."&sid=".$sid."&page=".$last;
        $return_arr['down']=$path."/information/document-outbox?qunzu=".$qunzu."&sid=".$sid."&page=".$next;
        $return_arr['end']=$path."/information/document-outbox?qunzu=".$qunzu."&sid=".$sid."&page=".$sum;
        
        $res=Attachment::find()->where(['sid'=>$sid,'type'=>$qunzu,'openid'=>$openid])->orderBy('create_time desc')->offset($star)->limit($num)->asArray()->all();
        // var_dump($res);
        $return_arr['list_msg'] =$res;
        $return_arr['count'] =$nm;
        $return_arr['totalPage'] =$totalPage;
        ...
        return $this->render('document-outbox',$return_arr);
    }
        /*  删除已发送信息 */
     public function actionDeleoutbox(){       
       $mid=\yii::$app->request->post("mid");
       $res= Attachment::findOne($mid)->delete();     
       if($res>0||$res===0){
          $this->ajaxReturn('success','json');
       }else{
          $this->ajaxReturn('fail','json');
     ...
     public function actionSearchdoc(){
        ...
        if(!empty($name)){
            $all=Attachment::find()->where(['sid'=>$sid,'type'=>$qunzu])->andWhere(['like','title',$name])->asArray()->all();
            if(!$all){
                $all=Attachment::find()->where(['sid'=>$sid,'type'=>$qunzu])->andWhere(['like','name',$name])->asArray()->all();
            }
        }else{
            $all=Attachment::find()->where(['sid'=>$sid,'type'=>$qunzu])->orderBy('create_time desc')->offset(0)->limit(5)->asArray()->all();
        }
        // var_dump($all);die;
     ...
     //监控视频
      ...
      public function actionDosendmsg(){
        // var_dump(\yii::$app->request->post());die;
         $strpath = \yii::$app->request->get("strpath");//附件
         $openid  = \yii::$app->view->params['openid'];//发送人  
         $sid = \yii::$app->view->params['sid'];//发送人 
         $des = $msg =\yii::$app->request->get("msg"); 
         $serid   = \yii::$app->request->get("serid");//语音id
         $title   = \yii::$app->request->get("title");  //消息主题  
         if(!empty($des)){
            $preg = "/<\/?[^>]+>/i";
         ...
         if($msg){
             ...
             $su->fujian=$strpath;
             if($school){
                $su->school=$school['name'];
             }else{
                $su->school='未绑定学校';
             }
             $su->uid=$this->getUid(); 
             $result=$su->save(false);
         }
