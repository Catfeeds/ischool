...
<!--            <foreach name="childs" item="vo">-->
            <?php foreach($childs as $k=>$v){?>
              <option id="<?php echo $v['sid'] ?>" value="<?php echo $v['school'] ?>|<?php echo $v['class'] ?>|<?php echo $v['stu_name'] ?>|<?php echo $v['sid'] ?>|<?php echo $v['stu_id'] ?>|<?php echo \yii::$app->view->params['openid']?>|<?php echo $v['stuno2']; ?>" name="<?php echo $v['endck'];?>"><?php echo $v['school'] ?>|<?php echo $v['class'] ?>|<?php echo $v['stu_name'] ?></option>
<!--            </foreach>-->
            <?php }?>
          </select>
        </div>
        <!--以上是学生姓名结构-->
        <input type="submit"  value="功能支付" id="btn" style="background: #6ec74f;">       
        <input type="submit" name="submit" id="btn2" value="学生补卡" style="background: #f39f1a">    
