...
//自写录音按钮
  RecordButton = (function(_super) {
    __extends(RecordButton, _super);

    function RecordButton() {
      _ref24 = RecordButton.__super__.constructor.apply(this, arguments);
      return _ref24;
    }

    RecordButton.prototype.name = 'record';

    RecordButton.prototype.icon = '';//'microphone'; 注释以免遮挡text属性

    RecordButton.prototype.text = '录音';

    RecordButton.prototype.title = '录音';
    
    RecordButton.prototype.needFocus = false;


    RecordButton.prototype.command = function() {
      ...
      if ($("#uloadfile-waper").css("display") == "block") {
  ...
  Simditor.Toolbar.addButton(RecordButton);

//自写录音按钮--------------------------------------------------------------

//上传图片按钮
  UploadImg = (function(_super) {
    __extends(UploadImg, _super);

    function UploadImg() {
      _ref25 = UploadImg.__super__.constructor.apply(this, arguments);
      return _ref25;
    }

    UploadImg.prototype.name = 'uploadimg';

    UploadImg.prototype.icon = 'picture-o';

    UploadImg.prototype.title = '上传图片';
    
    UploadImg.prototype.needFocus = false;


    UploadImg.prototype.command = function() {

      wx.chooseImage({
        success: function (res) {
          var localIds = res.localIds; // 返回选定照片的本地ID列表，localId可以作为img标签的src属性显示图片
          alert("本地ID："+localIds);
          $(".simditor-body").children("p").html("<img src='"+localIds+"'/>");
        }
      });
  ...
  Simditor.Toolbar.addButton(UploadImg);
...
//上传附件
  Uploadfile = (function(_super) {
    ...
    function Uploadfile() {
      _ref26 = Uploadfile.__super__.constructor.apply(this, arguments);
      return _ref26;
    }

    Uploadfile.prototype.name = 'uloadfile';

    Uploadfile.prototype.icon = '';//'paperclip'; 注释以免遮挡text属性

    Uploadfile.prototype.text = '附件';

    Uploadfile.prototype.title = '上传附件';
    
    Uploadfile.prototype.needFocus = false;


    Uploadfile.prototype.command = function() {

      $("#uloadfile-waper").slideToggle();

      if ($("#record-waper").css("display") == "block") {
        $("#record-waper").slideToggle();
      }else{
...
//上传附件--------------------------------------------------------------
  ...
  ImageButton = (function(_super) {
    ...
    ImageButton.prototype.text = '图片';

    ImageButton.prototype.icon = '';//'picture-o';注释掉以免遮挡text
    ...
    ImageButton.prototype.renderMenu = function() {
      ...
      createInput = function() {
        ...
        return $input = $('<input type="file" class="md" name="AidImg" title="上传图片" accept="image/*">').appendTo($uploadItem);
      };
      createInput();
      ...
      $(".md").localResizeIMG({
    		 ...
    		 success: function (result) {
    			 ...
    			 var url = _this.editor.opts.upload.url;
//    			 $('.loading').show();
//    			 $("#prevImg").attr("src",result.base64);
                         var shuzi=1;
                         var upload_p="图片加载"+shuzi+"%"+"，请稍候...";
                         var d = dialog({
                           title: '提示',
                           content: upload_p+'<span class="ui-dialog-loading" style="margin-top:10px">Loading..</span>',
                         });
    			 $.ajax({
    				 type: "POST",
    				 url: url,
    				 ...
    				 success: function(data){
    					time=setInterval(function(){
    				 	       	shuzi++;
                      upload_p="图片加载"+shuzi+"%"+"，请稍候...";
                      d.content(upload_p+'<span class="ui-dialog-loading" style="margin-top:10px">Loading..</span>');
                      if(shuzi==80){
                      	clearInterval(time);
                      }
    				 	  },50)
    					 ...
    					 var $block, $img, $nextBlock, range, name;
    				      if (name == null) {
    				        name = 'Image';
    				      }
    				      ...
    				      range = _this.editor.selection.getRange();
    				      range.deleteContents();
    				      $block = _this.editor.util.closestBlockEl();
    				      if ($block.is('p') && !_this.editor.util.isEmptyNode($block)) {
    				        $block = $('<p/>').append(_this.editor.util.phBr).insertAfter($block);
    				        _this.editor.selection.setRangeAtStartOf($block, range);
    				      }
    				      $img = $('<img/>').attr('src', data.file_path);
    				      range.insertNode($img[0]);
    				      $nextBlock = $block.next('p');
    				      if (!($nextBlock.length > 0)) {
    				        $nextBlock = $('<p/>').append(_this.editor.util.phBr).insertAfter($block);
    				      }
    				      _this.editor.selection.setRangeAtStartOf($nextBlock);
...
//        return _this.wrapper.removeClass('menu-on');
    	  $(this).find("input[type=file]").eq(0).trigger('loadLRZ');
      });
  ...
  ImagePopover = (function(_super) {
    ...
    ImagePopover.prototype._initUploader = function() {
      ...
      createInput = function() {
        ...
        return _this.input = $('<input type="file" title="上传图片" class="md" name="AidImg" accept="image/*">').appendTo($uploadBtn);
      };
      createInput();
