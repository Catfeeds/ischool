...
<header>
  <div class="container-fluid">
    <div class="row header-menu" id="header-menu1">
      ...
      <div class="col-xs-12">
       <div class="col-xs-3 jxt_op_btnol" id="back-btn" data-target="#Modal" data-toggle="modal" style="background-color:#777;">
              <a href="###"><span style="padding-right:5px;" class="glyphicon glyphicon-user"></span>成员</a>
         </div>
        ...
        <div class="col-xs-3 pull-right">
            <div name="<?php echo URL_PATH?>/group/sendmsg?openid=<?php echo \yii::$app->view->params['openid']?>&qunzu=<?= $qunzu;?>" class="jxt_op_btn" id="send_btn">写信</div>
        </div>
        <div class="col-xs-3 pull-right">
          <div name="<?php echo URL_PATH?>/group/inbox?openid=<?php echo \yii::$app->view->params['openid']?>&qunzu=<?= $qunzu;?>" class="jxt_op_btn jxt_current_btn" id="receive_btn">收信</div>
        </div>
        <div class="col-xs-3 pull-right">
          <div name="<?php echo URL_PATH?>/group/outbox?openid=<?php echo \yii::$app->view->params['openid']?>&qunzu=<?= $qunzu;?>" class="jxt_op_btn" id="sent_btn">已发</div>
        </div>
      </div>
    </div>
  </div>
</header>
 <div style="margin-top:20%;width:80%;margin-left:10%;" class="modal fade" id="Modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">
                       成员列表
                       <!-- <span style="font-size:1.5rem;font-weight:normal;">(支持下拉)</span> -->
                </h4>
            </div>
            <div class="modal-body">
               <?php if($alluser){?>
                    
                    <ul class="form-group text-center" style="max-height:260px;overflow:auto;"> 
                       <?php foreach($alluser as $k=>$v){?>   
                        <a style="width:100%;display:block;margin-top:5px;border-color:#ddd;" class="form-control" href="##"><span style="padding-right:5px;" class="glyphicon glyphicon-user"></span><?= $v['name']?></a>
                        <?php }?>
                    </ul>
...
//  var operate_type = $("#operate_type").val();
  ...
  if($.trim($("#txt-content").val())==""&&serid=="one"){
    ...
    var url=path+'/group/dosendmsg';
    var para={msg:msg,serid:serid,strpath:strpath,title:title,sid:$("#hidden_sid").val(),qunzu:$("#qunzu").val()};

    var to_url=path+"/group/outbox?openid="+$("#hidden_openid").val()+"&qunzu="+$("#qunzu").val();
