            ...
            <div class="tab-content">
                <div id="xiexin" class="tab-pane active">
                    <div class="btn-group" data-toggle="buttons-radio" style="margin-top: 10px">
                        <form style="display: inline-block" action = "/guanli/shenpi" method="post">
                        ...
                        <?php $index = 1; if(!empty($models['dataprovider'])) { foreach ($models['dataprovider'] as $key=>$value){?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a style="color: #36ADFF;"  href="/guanli/uploadsp?id=<?=$value['id']?>&tid=<?=$value['tid']?>&sid=<?=$value['sid']?>">下载</a><?php }else{ echo "空";}?></td>
                    ...
                    <table class="table table-bordered text-center" style="margin-top: 20px;">
                        ...
                        <?php $index = 1; if(!empty($sjxmodels['dataprovider'])) { foreach ($sjxmodels['dataprovider'] as $key=>$value){?>
                            <tr>
                                ...
                                <td><?php if(!empty($value['work']['fjurl'])){ ?><a style="color: #36ADFF;"  href="/guanli/uploadsp?id=<?=$value['work_id']?>&tid=<?=$value['work']['tid']?>&sid=<?=$value['work']['sid']?>">下载</a><?php }else{ echo "空";}?></td>
...
<div class="modal fade" id="gzModal">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form  id="antoform" class="form-horizontal calender" role="form" action="/guanli/shenpi" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-3">
                            <select class="form-control" onchange="changeFenzu(this.value)">
                                ...
                                <?php var_dump($info['fenzu']);if (!empty($info['fenzu'])) {foreach ($info['fenzu'] as $v){?>
...
<script>
    ...
    function xiexinpa(){
        ...
        window.location.href = "/guanli/shenpi?type=xiexin";
    }
    function shouxinpa(){
        var url = window.location.href;
        window.location.href = "/guanli/shenpi?type=shouxin";
    }
    //上传报表信息
    function tianjia(){
        var title = $("#zhuti").val();
