<div class="pull-left" style="margin-left: 10px;width: 70%;min-width: 600px;">
    ...
    <ul class="list-group" style="background-color: white;padding: 10px 20px;margin-top: 5px;box-shadow: 0 0 2px #ccc;">
        <li style="border: none;line-height: 30px;" class="list-group-item">
            <span class="gg">公告</span><?= !empty($gonggao)?$gonggao[0]['title']:"暂无公告信息！";?><?php if(!empty($gonggao)){ ?><a class="pull-right" href="/teacher/schoolmin?id=<?=$gonggao[0]['id'];?>">>>查看详情</a><?php }?>
        </li>
        <li style="border: none;border-top: 1px solid #ddd;line-height: 30px;" class="list-group-item">
            <span style="background-color: #FF9900;" class="gg">动态</span><?= !empty($dongtai)?$dongtai[0]['title']:"暂无动态信息！";?><?php if(!empty($dongtai)){ ?><a class="pull-right" href="/teacher/dongtaimin?id=<?=$dongtai[0]['id'];?>">>>查看详情</a><?php }?>
        </li>
    </ul>
        ...
        <?php foreach($columns as $key=>$value){?>
            <div class="zhanshi">
                ...
                <div>
                    ...
                    <a class="xgk_xq" href="/teacher/websitemin?id=<?=$value['id']?>">>>查看详情</a><?php } ?>
                </div>
            </div>
        <?php } ?>
    </div>
