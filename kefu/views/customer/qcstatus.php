...
<!-- <form action="/customer/up" method="post" enctype="multipart/form-data" target="hidden_frame"> -->
<form id="search" class="form-horizontal" action="/customer/qcstatus" method="post">
...
<div style="max-width:1000px;margin:20px auto;" class="form-group">
    ...
    <div class="clear" style="padding-left:10%; margin:15px 0px;">
    	<input type="button"  class="btn btn-primary float-left"  onclick="search_result()" value="查询" />
    </div>
...
<br/>
<div style="max-width:1000px;margin:20px auto;" class="form-group">
	    <div  style="padding-left:10%; margin:15px 0px;color:red;">
    	 提示：如通过单号查询，姓名、学号两项可不填。
         </div> 
	     <label class="col-sm-2 control-label " for="stuname" style="text-align:right;margin-top:6px;">姓&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;名:</label>
         <div class="col-sm-6">
		 ...
		 <label class="col-sm-2 control-label " for="stuno2" style="text-align:right;margin-top:6px;">学&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;号:</label>
         <div class="col-sm-6">
           <input type="text" id="stuno2" class="form-control" name="stuno2" placeholder="请输入学号,例（T566501234567/5312345678)">
		 </div>
		  <div class="clearfix"></div>
		  <br/>
...
<table class="text-center table table-bordered " width="100%" style="max-width:1000px;margin:0 auto;" id="consume">
	<tr id="ckcode" >
		<td style="border-color:#aaa;" colspan="6">消费记录</td>
...
<script>
	...
	function search_consume(){
		...
		$.ajax({
				...
				data:'starttime='+starttime+'&endtime='+endtime+'&card_no='+card_no+'&school_id='+school_id+'&stuname='+stuname+'&stuno2='+stuno2,
				dataType:'json',
				type:'post',
				success:function(data){					
					if(data.flag==0){					                       
					    $.each(data.ckshuju,function(i,info){
						var tr = "<tr><td style='border-color:#aaa;'>"+info.card_no+"</td><td style='border-color:#aaa;'>"+info.school_name+"</td><td style='border-color:#aaa;'>"+info.amount+"</td><td style='border-color:#aaa;'>"+info.balance+"</td><td style='border-color:#aaa;'>"+info.time+"</td><td style='border-color:#aaa;'>"+info.phyid+"</td></tr>";	
						$("#consume").append(tr);
					    })
